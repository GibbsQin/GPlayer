...
static void av_format_init_audio(AVFormatContext *ifmt_ctx, AVStream *stream) {
    ffmpeg_extra_audio_info(ifmt_ctx, stream, mediaInfo);
    if (mediaInfo->videoType != 0) {
...
static void av_format_init_video(AVFormatContext *ifmt_ctx, AVStream *stream) {
    ffmpeg_extra_video_info(ifmt_ctx, stream, mediaInfo);
    if (mediaInfo->audioType != 0) {
        stream_media_callback.av_init(streamChannelId, mediaInfo);
    }
}
...
static uint32_t av_format_feed_video(AVFormatContext *ifmt_ctx, AVPacket *packet) {
    AVRational time_base = ifmt_ctx->streams[packet->stream_index]->time_base;
    return stream_media_callback.av_feed_video(streamChannelId, packet->data, packet->size,
                                               ffmpeg_pts2timeus(time_base, packet->pts),
...
int create_stream_channel(const char *url, const struct MediaCallback callback) {
    ...
    ffmpeg_callback.av_format_init_audio = av_format_init_audio;
    ffmpeg_callback.av_format_init_video = av_format_init_video;
